...
exports.createLocationRegistration = function (newLocationRegistration, res, callback) {
                ...
                if (pending == 0) {
                    if (fewRegistrations.length != 0) {
                        if (fewRegistrations[0].locationsRegistrations[0].raceScore != 0) {
                            fewRegistrations.sort(function (a, b) { return (a.locationsRegistrations[0].raceScore >= b.locationsRegistrations[0].raceScore) ? -1 : 1; });
                            for (i = 0; i < fewRegistrations.length; i++) {
                                fewRegistrations[i].placement = i + 1;
                            }
                        }
                        else {
                            fewRegistrations.sort(function (a, b) { return (a.shipId > b.shipId) ? 1 : -1; });
                        }
                    }
...
exports.getScoreboard = function (req, res) {
    ...
    EventRegistration.find({ eventId: req.params.eventId }, { _id: 0, __v: 0 }, function (err, eventRegistrations) {
        ...
        if (eventRegistrations.length !== 0) {
            var scores = [];
            eventRegistrations.forEach(function (eventReg) {
                pending++;
                                ...
                                if (pending === 0) {
                                    if (scores.length != 0) {
                                        if (scores[0].locationsRegistrations[0].raceScore != 0) {
                                            scores.sort(function (a, b) { return (a.locationsRegistrations[0].raceScore >= b.locationsRegistrations[0].raceScore) ? -1 : 1; });
                                            for (i = 0; i < scores.length; i++) {
                                                scores[i].placement = i + 1;
                                            }
                                        }
                                        else {
                                            scores.sort(function (a, b) { return (a.shipId > b.shipId) ? 1 : -1; });
                                        }
                                    }
...
exports.getReplay = function (req, res) {
    EventRegistration.find({ eventId: req.params.eventId }, { _id: 0, __v: 0 }, function (err, eventRegistrations) {
        if (err) {
            return res.status(500).send({ message: err.message || "Some error occurred while retriving eventRegistrations" });
        }
...
exports.deleteFromEventRegId = function (req, res) {
    // Checking if authorized 
    Auth.Authorize(req, res, "user", function (err) {
        if (err)
            return err;
